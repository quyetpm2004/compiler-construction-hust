...
void compileVarDecls(void)
{
  if (lookAhead->tokenType == KW_VAR)
  {
    ...
    do
    {
      ...
      eat(TK_IDENT);
      checkFreshIdent(currentToken->string);
      variables[n++] = strdup(currentToken->string);
      ...
      for (int i = 0; i < n; i++)
      {
        Object *varObj = createVariableObject(variables[i]);
        varObj->varAttrs->type = duplicateType(varType);
...
void compileStatement(void)
{
  ...
  case KW_BREAK:
    if (loopLevel == 0)
      error(ERR_BREAK_NOT_IN_LOOP, lookAhead->lineNo, lookAhead->colNo);
    eat(KW_BREAK);
    break;
  case KW_CONTINUE:
    if (loopLevel == 0)
      error(ERR_CONTINUE_NOT_IN_LOOP, lookAhead->lineNo, lookAhead->colNo);
    eat(KW_CONTINUE);
    break;
