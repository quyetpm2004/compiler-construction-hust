...
void compileForSt(void)
{
  ...
  eat(SB_ASSIGN);
  expType = compileExpression();
  checkIntType(expType); // Giá trị khởi đầu phải là số nguyên

  eat(KW_TO);
  expType = compileExpression();
  checkIntType(expType); // Giá trị kết thúc phải là số nguyên
...
void compileArgument(Object *param)
{
  ...
  // Kiểm tra kiểu giữa tham số và đối số
...
void compileCondition(void)
{
  ...
  // Phân tích vế trái và lấy kiểu dữ liệu của nó
...
Type *compileIndexes(Type *arrayType)
{
  ...
  {
    eat(SB_LSEL);
    // Mảng phải tồn tại mới được truy chỉ số
    checkArrayType(type);
    Type *idxType = compileExpression();
    checkIntType(idxType); // Chỉ số mảng phải là số nguyên
    eat(SB_RSEL);
